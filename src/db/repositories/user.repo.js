...
class UserRepository extends BaseRepository {
	async update(id, body) {
		const { roles, cinemaId, password, ...filtered } = body;
		if (password) {
	...
	async getAll(queryOptions) {
		return this.getPaginated(queryOptions).then((response) => ({
			...response,
			data: response.data.map((user) => this.userWithoutPassword(user)),
		}));
	}
	...
	async getAllManagers({ skip = 0, limit = 15 } = {}) {
		...
		return this.getPaginated({ skip, limit, queryBody: managerQuery }).then(
			(response) => ({
				...response,
				data: response.data.map((user) => this.userWithoutPassword(user)),
			})
